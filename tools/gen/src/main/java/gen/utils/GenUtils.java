...
import gen.entity.ColumnEntity;
import gen.entity.TableEntity;
...
//		templates.add("template/list.js.vm");
	...
	public static void generatorCode(Map<String, String> table,
		...
		String oldTableName = table.get("tableName");
		String[] nameStrs = oldTableName.split("_");
		if(nameStrs.length > 1 && nameStrs[0].length() == 1) {
//		if(nameStrs.length > 1) {
			oldTableName = oldTableName.substring(nameStrs[0].length() + 1,oldTableName.length());
		}
		for(int i=1;i<nameStrs.length;i++) {
			//取第一个字母，例如t_abc_def，别名为ad
			alias = alias + nameStrs[i].substring(0, 1);
		}
		tableEntity.setAlias(alias);
		tableEntity.setComments(table.get("tableComment"));
		//表名转换成Java类名
		String className = tableToJava(oldTableName, config.getString("tablePrefix"));
		...
		for(Map<String, String> column : columns){
			...
			if("Date".equals(column.get("dataType"))) {
				columnEntity.setHasDate(true);
				tableEntity.setHasDate(true);
			}
			if("BigDecimal".equals(column.get("dataType"))) {
			...
			//列名转换成Java属性名
			String attrName = "";
			if(columnEntity.getColumnName().indexOf("_") > -1){
				attrName = columnToJava(columnEntity.getColumnName());
			}else{
				attrName = columnEntity.getColumnName();
			}
		...
		map.put("alias", tableEntity.getAlias());
...
//			tableName = tableName.replace(tablePrefix, "");
	...
	public static String getLocalFileName(String template, String className, String packageName){
		...
		if(template.contains("Entity.java.vm")){
			return config.getString("entity") + File.separator + className + ".java";
		}

		
		if(template.contains("Dao.java.vm")){
			return config.getString("dao") + File.separator + className + "Mapper.java";
		}
		...
		if(template.contains("list.html.vm")){
			return config.getString("html")+ File.separator + className.toLowerCase() + "List.html";
		}
		if(template.contains("add.html.vm")){
			return config.getString("html") + File.separator + "add" +className + ".html";
		}
		if(template.contains("update.html.vm")){
			return config.getString("html") + File.separator + "update" + className + ".html";
		}
		if(template.contains("Req.java.vm")){
			return config.getString("param") + File.separator  +className + "ReqVO.java";
		}
		if(template.contains("Resp.java.vm")){
			return config.getString("param") + File.separator  + className + "RespVO.java";
		}
